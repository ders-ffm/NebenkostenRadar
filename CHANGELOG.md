...
**Korrektur einer falschen Aussage:** Ich hatte behauptet, in dieser Umgebung keine Bild-Grafiken (PNG-Karten) erzeugen zu können und dafür Canva/Figma vorgeschlagen — Stefan hat zu Recht widersprochen, das bestehende Bildmaterial sei ohne externe Programme entstanden. Geprüft: Playwright/Chromium scheitert im Sandbox ohne Root-Rechte an fehlenden Systembibliotheken, aber Python/Pillow mit lokal installierten Poppins-Schriftarten funktioniert und trifft den bestehenden Stil (Farben aus `theme.js`, Poppins Bold für Überschriften) sehr nah. Dabei außerdem entdeckt: von den als "unverändert bestehend" angenommenen Fakt-Karten war nur noch `post_5_fakt_heizkosten-50-70` tatsächlich im Ordner vorhanden — fünf weitere (post_4, 7, 8, 9, 10) fehlten ebenfalls. Alle fünf plus sechs neue Werbe-Karten plus zwei neue Steuer-Bonus/Verwaltungskosten-Karten (13 PNGs insgesamt) neu erzeugt, `content-kalender.md` mit den echten Dateinamen aktualisiert.

**Neue Dokumentation:** `planung/projektdokumentation-nkr.md` (Gesamtüberblick Produkt/Tech/Recht/Business) und `planung/werbeplan-nkr.md` (Kanäle, Budget, Saisonplan, KPIs, bewusst nicht verfolgte Ansätze) neu angelegt — beide bestanden noch nicht. `marketing/meta-kampagne/anleitung-instagram-reels.md` (drei fertige Skripte + Technik-Anleitung für Reel-Einsteiger) und ein neuer GA4-Auswertungs-Abschnitt in `anleitung-meta-ads-kampagne.md` ergänzt. `NebenkostenRadar_Steuerleitfaden_2026.docx` um Abschnitt "3.6 Marketing & Werbung (Meta-Ads)" erweitert (Flight 1 = 120 € tatsächlich gebucht, Flight 2 = ~80 € nur geplant, nicht mitgerechnet), Gesamtsumme auf ~€1.033 aktualisiert. `README.md` Zeile 3 hatte noch "rechtssicher" stehen (gleicher Fund wie 19.08. in `Welcome.jsx`) — korrigiert.
...
Dazu 6 weitere, sofort einsetzbare Posts über tatsächlich gemeisterte Hürden in diesem Projekt, jetzt **chronologisch nach dem realen Zeitpunkt der jeweiligen Geschichte sortiert** (nicht nach Post-Nummer-Belieben): der Kurswechsel kurz vor dem geplanten Start (Handeingabe erwies sich beim Testen mit echten Abrechnungen als unbrauchbar, da jeder Vermieter anders strukturiert — Umbau auf Foto-/PDF-Upload, früheste Geschichte, kein exaktes Datum im Repo dokumentiert), der zweimal gefixte CO2-Kosten-Doppelzählungs-Bug (12.–13.08.2026), der Live-Timeout-Bug bei der Foto-Erkennung (13.08.2026), die unterschätzte Widerrufsrecht-Pflicht samt Nachweispflicht (13.08.2026), die falsche erste Vermutung bei den Google-Indexierungsproblemen (13.08.2026, tote Sitemap-Links statt Duplikate) und zuletzt, als aktuellster Stand, konkrete Maßnahmen für organisches Wachstum ohne Werbebudget. Jeder Post endet jetzt außerdem mit einer themenbezogenen, offenen Frage zum Erfahrungsaustausch (auf Stefans Wunsch: "wer kennt das", "wie macht ihr das") statt rein einseitig zu senden.
