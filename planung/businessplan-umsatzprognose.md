...
- **nebify**: Sehr konkrete Social Proof: "Unsere Fälle" mit als "Originaldokument, personenbezogene Angaben anonymisiert" beschrifteten Prüfergebnissen (Betrag, Datum, Stadt, konkrete Rechtsgrundlage). Interaktive Produkt-Vorschau direkt auf der Startseite (Beispiel-Prüfbericht mit Zahlen). Breite Zahlungsmethoden über Stripe (Visa, Mastercard, Amex, Apple Pay, Google Pay, PayPal, Klarna, SEPA) — deutlich mehr als NKRs zwei Payment Links. Geld-zurück-Garantie ist zentrales Werbeversprechen, mehrfach wiederholt.

**Nachtrag 24.08.2026, Echtheit der Fallbeispiele geprüft (Stefans Verdacht):** Zwei "Unsere Fälle"-Dokumente stichprobenartig via Screenshot geprüft (Fälle Straubing und Itzehoe). Beide zeigen dasselbe generische Layout: getippter Text ohne Briefkopf, ohne Firmenlogo der Hausverwaltung, ohne Unterschrift (nur ein geschwärztes Feld), dieselbe dekorative "angerissenes Papier"-Rahmengrafik links/rechts, generische Positionsbezeichnungen wie "Abzug 1", "Abzug 2", "Abzug 3" statt realer Kostenarten. Das ist ein einheitliches Design-Template, keine fotografierten oder gescannten echten Abrechnungen — die Beschriftung "Originaldokument" ist damit nach dieser Stichprobe irreführend. Das bestätigt nicht automatisch, dass die zugrundeliegenden Fälle komplett erfunden sind (die Beträge könnten aus echten Fällen stammen und nur nachgestellt/anonymisiert dargestellt werden), widerlegt aber den Anschein einer echten Dokumenten-Reproduktion. Für NKR bedeutet das: Bei eigenen künftigen Fallbeispielen entweder echte (geschwärzte) Originalscans zeigen oder ehrlich als "nachgestelltes Beispiel" kennzeichnen — nicht wie nebify den Anschein eines Originaldokuments erwecken, wenn es keins ist.

**Nachtrag 24.08.2026, Marktalter geprüft (Stefans Verdacht "nicht länger am Markt als wir"):** Wayback-Machine-Historie von nebify.de zeigt 15 Captures zwischen 17.05.2024 und 21.03.2025, danach keine weiteren Captures. Mai 2024: Domain leitete auf eine passwortgeschützte Seite (`/password`) um — reine Platzhalter-Domain, kein Betrieb. März 2025: Seitentitel im Archiv lautet "nebify-checkerboard" — ebenfalls ein Platzhalter-/Template-Zustand, keine fertige Seite. Die auf der Seite selbst veröffentlichten Fallbeispiele beginnen erst am 13.11.2025. Zusammengenommen spricht das dafür, dass nebify die Domain zwar schon 2024 reserviert hatte, der tatsächliche Geschäftsbetrieb aber vermutlich erst deutlich später (2025) begann — nicht abschließend beweisbar, da nach März 2025 keine weiteren Wayback-Daten vorliegen. Für einen fairen Vergleich mit NKRs eigenem Start fehlt mir Stefans genaues NKR-Startdatum (die Projekt-CHANGELOG.md dokumentiert erst ab 25./26.07.2026 durchgängig, das muss aber nicht der tatsächliche Projektstart sein) — offene Rückfrage an Stefan.
...
**Nachtrag 24.08.2026, Umsatzgrößenordnung NebenkostenPro und nebify geprüft (Mineko ausgenommen, dazu keine belastbare Quelle gefunden):**

Fakten, direkt aus den Impressen und Trustpilot geprüft:

- NebenkostenPro wird von Julian Falk als Einzelunternehmer betrieben (Fürth), nicht als GmbH. Das Impressum weist ausdrücklich auf § 19 UStG hin ("Gemäß § 19 UStG wird keine Umsatzsteuer ausgewiesen") — das ist die gesetzlich vorgeschriebene Angabe für die Kleinunternehmerregelung.
- Derselbe Julian Falk betreibt unter identischem Impressum auch BußgeldPro (bussgeld-pro.de). Auf beiden Seiten wird zusätzlich auf MietCheck, ArbeitsvertragCheck und WegCheck als "weitere Tools" verlinkt — ein Portfolio mehrerer Legal-Tech-Nischentools einer Person, nicht nur ein Produkt.
- NebenkostenPro auf Trustpilot: 3,5 Sterne, 10 Bewertungen (Stand 24.08.2026).
- nebify auf Trustpilot: 4,0 Sterne, 3 Bewertungen (Stand 24.08.2026).
- nebifys Impressum (Kolibri eCommerce GbR) enthält keinen § 19 UStG-Hinweis.
- Für keinen der beiden Anbieter existieren veröffentlichte Umsatzzahlen — weder GbR noch Einzelunternehmen dieser Größe müssen Jahresabschlüsse beim Bundesanzeiger einreichen.

Einordnung, ausdrücklich als Schlussfolgerung und nicht als Faktum markiert:

- Die § 19 UStG-Angabe bei NebenkostenPro bedeutet rechtlich, dass Julian Falks Gesamtumsatz über **alle** seine Tools zusammen (nicht nur NebenkostenPro) unter der Kleinunternehmergrenze liegt (Stand Reform 2025: ≤ 25.000 € Vorjahresumsatz und ≤ 100.000 € laufendes Jahr). Die auf der Seite beworbenen "1.800+ zufriedene Kunden" sind damit wahrscheinlich überwiegend kostenlose Vorschau-Nutzer, nicht 1.800 zahlende Kunden — allein 1.800 × 7,90 € läge bereits bei 14.220 € aus einem einzigen von fünf Tools.
- Das Fehlen der § 19 UStG-Angabe bei nebify spricht für Regelbesteuerung und damit vermutlich für einen höheren Umsatz als bei NebenkostenPro. Eine belastbare Zahl lässt sich daraus aber nicht ableiten, nur eine Größenordnungs-Tendenz.
- Die niedrigen Trustpilot-Bewertungszahlen (10 bzw. 3) sind ein Indiz für ein kleines Kundenvolumen, kein Beweis für die exakte Kundenzahl, da die Bewertungsquote je nach Anbieter stark schwankt.

**Was NKR daraus ableiten kann:** Beide hier geprüften Wettbewerber (Mineko ausgenommen) sind nach dieser Evidenz vermutlich kleine, ein- bis zweiköpfige Betriebe, keine finanzstarken, groß skalierten Unternehmen. Das relativiert den Eindruck von Überlegenheit, den ihre Marketingseiten (Kundenzähler, Fallbeispiele, Produktbreite) vermitteln. NKRs eigenes Ziel von 250–1.000 Verkäufen/Jahr (Abschnitt 9) wirkt vor diesem Hintergrund nicht unrealistisch niedrig — selbst ein Wettbewerber mit fünf Tools, längerer Marktpräsenz und einem "1.800+"-Kundenzähler bewegt sich offenbar in einer ähnlichen Umsatzgrößenordnung. Separat zu bewerten (keine Empfehlung, nur Beobachtung): Julian Falks Strategie, dieselbe technische Basis auf mehrere angrenzende Nischen (Bußgeld, Miete, Arbeitsvertrag, WEG) zu verteilen, ist ein reales Muster bei einem direkt vergleichbaren Solo-Betrieb, unabhängig davon, ob es für NKR sinnvoll wäre.
...
**Interessanter Fund bei nebify, unabhängig von der Wettbewerbsfrage:** nebify zitiert für die "jede zweite Abrechnung fehlerhaft"-Zahl eine konkrete Pressemitteilung des Mieterverein Düsseldorf e.V. vom 08.10.2025 als Primärquelle (zusätzlich zum Deutschen Mieterbund). Das könnte die bisher fehlende exakte Primärquelle für unsere eigene Nutzung dieser Zahl sein — ich konnte den Link aber nicht selbst abrufen (Fetch ist zweimal getimeoutet), das ist also nebifys Behauptung, nicht von mir verifiziert. Bevor wir unsere eigene Quellenangabe darauf stützen, sollte das jemand mit funktionierendem Zugriff direkt gegenprüfen.
