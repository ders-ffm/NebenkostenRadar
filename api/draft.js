// ─────────────────────────────────────────────────────────────────────────
// draft.js — NEU (30.08.2026, siehe projektdokumentation-nkr.md Abschnitt 9,
// UX-Test-Nachtrag). Speichert und liest einen NICHT bezahlten Zwischen-
...
//
// TECHNISCHER HINWEIS (31.08.2026): Ursprünglich zwei getrennte Dateien
// (save-draft.js für POST, get-draft.js für GET). Zusammengeführt, weil der
// Vercel-Hobby-Plan maximal 12 Serverless Functions pro Deployment erlaubt
// ("No more than 12 Serverless Functions can be added to a Deployment on
// the Hobby plan.") — zwei neue Dateien haben dieses Limit gerissen, eine
// neue Datei mit Methoden-Verzweigung (POST/GET) kostet nur einen Slot.
// Rein organisatorische Änderung, keine Funktionsänderung.
//
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!supabaseRes.ok) {
        ...
        console.error("Supabase Fehler (draft POST):", err);
        return res.status(500).json({ error: "Speichern fehlgeschlagen" });
      }
      return res.status(200).json({ success: true });
    } catch (err) {
      console.error("Fehler (draft POST):", err.message);
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      console.error("Fehler (draft GET):", err.message);
      return res.status(500).json({ error: err.message });
    }
  }
