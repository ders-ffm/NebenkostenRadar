...
- [ ] **Stripe-Testmodus aktivieren (kein echtes Geld, getrennt vom Live-Betrieb):**
  1. Stripe-Dashboard oben rechts auf "Test mode" umschalten.
  2. Im Testmodus zwei neue Payment Links anlegen (gleiche Einstellungen wie live: 7,99€/9,99€, Gutscheincodes zulassen, Erfolgsseite `https://nebenkostenradar.com/pruefen/download?session={CHECKOUT_SESSION_ID}`) — Payment Links sind an den Modus gebunden, die Live-Links funktionieren im Testmodus nicht.
  3. Weiterhin im Testmodus: Developers → Webhooks → Add endpoint → URL `https://nebenkostenradar.com/api/webhook`, Event `checkout.session.completed` → Test-`whsec_...` kopieren.
  4. Developers → API keys (Testmodus) → `sk_test_...` kopieren.
  5. In Vercel temporär: `STRIPE_SECRET_KEY` → der `sk_test_...`-Wert, `STRIPE_WEBHOOK_SECRET` → der Test-`whsec_...`-Wert. Vercel deployt automatisch neu.
  6. Testkauf mit dem Test-Payment-Link, Testkarte `4242 4242 4242 4242`, beliebiges zukünftiges Datum, beliebige 3-stellige CVC, beliebige PLZ. E-Mail-Felder dabei bewusst einmal falsch wiederholen (Fehlermeldung muss erscheinen), dann korrekt ausfüllen.
  7. **Danach nicht vergessen:** `STRIPE_SECRET_KEY` und `STRIPE_WEBHOOK_SECRET` in Vercel wieder auf die Live-Werte zurücksetzen, bevor echte Kunden die Seite nutzen — sonst laufen echte Zahlungsversuche gegen den Testmodus.
